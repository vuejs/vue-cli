...
module.exports = function generate (name, src, dest, done) {
  ...
  if (delims && Array.isArray(delims) && delims.length === 2) {
    if (delims[0].indexOf('=') === -1) {
      delims[0] = delims[0] + '(?!=)'
    }

    reDelimiters = new RegExp(delims[0] + '([\\s\\S]+?)' + delims[1], 'g')
    require('handlebars-delimiters')(Handlebars, delims)
  }
...
function renderTemplateFiles (skipInterpolation) {
  ...
  return function (files, metalsmith, done) {
    ...
    async.each(keys, function (file, next) {
      ...
      // do not attempt to render files that have no handlbers block
      if (!reDelimiters.test(str)) {
        return next()
      }
