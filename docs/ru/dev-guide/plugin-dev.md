...
Хук `onWrite` принимает множество [аргументов](ui-api.html#сохранение-изменений-конфигурации), но нам необходимо только два из них: `prompts` и `api`. В первом текущие объекты интерактивных подсказок — мы получим id интерактивной подсказки и ответ для этого id. Для получения ответа воспользуемся методом `async getAnswer()` из `api`:
...
Объект подсказки можно расширить дополнительными свойствами. Они опциональны и используются только в UI:
