...
- По умолчанию: `'/'`
  ...
  Значение также может быть пустой строкой (`''`), чтобы все ресурсы подключались через относительные пути, что позволит в таком случае использовать эту сборку в окружении файловой системы, например в гибридном приложении Cordova. **Обратите внимание, что это ограничит расположение сгенерированных CSS файлов только корневым каталогом сборки, чтобы гарантировать правильность работы ссылок в вашем CSS.**
  ...
  module.exports = {
    pages: {
      index: {
        ...
        title: 'Index Page',
        // все чанки, добавляемые на этой странице, по умолчанию
        // это извлеченные общие чанки и вендорные чанки.
...
- По умолчанию: `true`
  ...
  ::: tip Совет
  Чтобы ошибки линтинга отображались в браузере, установите в значение `'error'`.
  :::
  ...
  Когда значение `true`, eslint-loader будет выдавать предупреждения (которые логируются только в терминале, чтобы не отвлекать во время разработки). Если необходимо в таких случаях генерировать ошибки, установите в значение `'error'`. Тогда при наличии ошибок линтинга они будут отображаться в браузере, а процесс сборки завершаться с ошибкой.
