...
- Тип: `boolean`
- По умолчанию: `true` (в режиме production, всегда `false` в других случаях)
  ...
  Извлечение CSS всегда отключено в режиме `development` так как оно мешает работе горячей замены модулей.
