...
::: tip Примечение для webpack 4
При использовании `webpack` 4-ой версии, которая является стандартной для Vue CLI 4, Вам нужно убедиться, что Ваши загрузчики совместимой с `webpack` версии. Иначе Вы получите ошибку с сообщением о конфликтующих зависимостях. В таком случае Вы можете использовать более старую версию загрузчика, которая по-прежнему совместима с `webpack` 4.
