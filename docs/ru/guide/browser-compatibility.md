...
::: tip Совет
`<script type="module">` загнружаетс [при всегда включённом CORS](https://jakearchibald.com/2017/es-modules-in-browsers/#always-cors). Это означает, что ваш сервер должен возвращать корректные заголовки CORS, такие как `Access-Control-Allow-Origin: *`. Если вы хотите загружать скрипты с помощью credentials, используйте опцию [corsUseCredentials](../config/#corsusecredentials).
:::
