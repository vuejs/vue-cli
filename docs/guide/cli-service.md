...
`[entry]` in the CLI is defined as `the entry file`, not `an additional entry file`. If you overwrite the entry in the CLI, then the entries from `config.pages` are no longer considered. This may cause an error.
